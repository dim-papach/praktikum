    ...
    # https://pardiso-project.org/r-inla/ 
    # We use R-INLA package PARDISO, a high-performance 
    # software for solving sparse symmetric that 
    # arises in the R-INLA approach to Bayesian inference.
    #
    #  path to the PARDISO license key
    library(INLA)
    ...
    input_post = array_reshape(input_pre,
        ...
        scores<-pca$x[,1:pca.num]
        
        scores_original<-array_reshape(scores,
                                       dim = c(dims[2], dims[1], pca.num))
        ...
        input_Net <- array(NaN, dim = c(spatial_size,pca.num)) 
        ...
        ####################################################
        time_holder <- array(NaN, dim = pca.num)       #
        INPUT_PRE_arr <- array(NaN, dim = c(dims[2], dims[1], pca.num))
        INPUT_POST_arr <- array(NaN, dim = c(dims[2], dims[1], pca.num))
        MEAN_arr <- array(NaN, dim = c(dims[2], dims[1], pca.num))
        SD_arr <- array(NaN, dim = c(dims[2], dims[1], pca.num))
        ...
        #  for(s in 1:1){##
          ...
          print(paste0("End of iteration ",s," out of ", pca.num))
        ...
        pca_pre <- MEAN_arr[1:dims[1],1:dims[2],1:pca.num]
        pca_post <- array_reshape(pca_pre, dim = c(input_dataset_size,pca.num))
        
        #reorientation 
        pca<-readRDS('pca.rds')
        
        pca_recon <- pca_post[,1:pca.num] %*% t(pca$rotation[,1:pca.num])
        if(pca$scale != FALSE){
          pca_recon <- scale( pca_recon, center = FALSE , scale=1/pca$scale)
        }
        if(pca$center != FALSE){
          pca_recon <- scale( pca_recon, center = -1 * pca$center, scale=FALSE)
        }  
        
        pca_recon = array_reshape(pca_recon,
        ...
        writeFITSim(pca_recon, file = paste0('pca+inla_Mean.fits'))
        ...
        write.csv(time_holder,"time_pca.csv", row.names = FALSE)
