...
** Saving the masked FITS

#+begin_src python
mask = masks[2]
h_masked = np.ma.masked_where(mask, h_data)
n_masked = np.ma.masked_where(mask, n_data)
o_masked = np.ma.masked_where(mask, o_data)
h_masked_filled = h_masked.filled(np.nan)

# Mask and save H-alpha data
h_masked_filled = h_masked.filled(np.nan)
h_hdu = fits.PrimaryHDU(h_masked_filled)
h_hdul = fits.HDUList([h_hdu])
h_hdul.writeto('HI_6563s_masked.fits', overwrite=True)

# Mask and save OI data
o_masked = np.ma.masked_where(mask, o_data)
o_masked_filled = o_masked.filled(np.nan)
o_hdu = fits.PrimaryHDU(o_masked_filled)
o_hdul = fits.HDUList([o_hdu])
o_hdul.writeto('O1_6300s_masked.fits', overwrite=True)

# Mask and save NII data
n_masked = np.ma.masked_where(mask, n_data)
n_masked_filled = n_masked.filled(np.nan)
n_hdu = fits.PrimaryHDU(n_masked_filled)
n_hdul = fits.HDUList([n_hdu])
n_hdul.writeto('N2_6583s_masked.fits', overwrite=True)

print("Masked FITS files saved as 'HI_6563s_masked.fits', 'O1_6300s_masked.fits', and 'N2_6583s_masked.fits'")
#+end_src
...
n_fill = maskfill.maskfill(n_masked.data, n_masked.mask,writesteps=False,output_file='N_fill.fits',verbose=True)
#+end_src
...
fill_vis(n_masked, n_masked.data, n_fill, "NII_fill", "NII")
#+end_src
